	...
	return <li className="rsvp-row" aria-labelledby={`${id}-name`}>
		<div className="rsvp-row-main">
			<div className="rsvp-person"><p id={`${id}-name`}>{displayedName}</p>
				...
				{(open || replacement) && hasName && <p className={replacement && !forbiddenReplacement && isValidGuestName(name) ? 'rsvp-note rsvp-replacement-status' : 'rsvp-note'}>{forbiddenReplacement ? 'Reemplazo no disponible' : isValidGuestName(name) ? '✓ Asistirá' : 'Nombre por completar'}</p>}
			</div>
		...
		{hasName && (open || replacement) && <div className={replacement ? 'rsvp-row-actions rsvp-replacement-actions' : 'rsvp-row-actions'}>
			...
			{open && <button id={`${id}-clear`} type="button" className="rsvp-action"
				onClick={() => returnFocus(onClear)}>Dejar libre</button>}
			{replacement && <button id={`${id}-clear`} type="button" className="rsvp-action rsvp-restore"
				aria-describedby={forbiddenReplacement ? `${id}-unavailable` : undefined} onClick={() => returnFocus(onRestore)}>
				<span className="rsvp-restore-label"><Undo2 size={14} aria-hidden="true" />Restaurar invitado original</span>
			</button>}
		</div>}
		<div id={`${id}-editor`}>
			<AnimatePresence initial={false}>
				{expanded && canName && <GuestNameEditor key="editor" id={id} value={name} isReplacement={!open}
